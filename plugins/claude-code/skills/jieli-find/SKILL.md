...
3. Do not pass --provider based on the current agent. Do not set `--provider claude_code` merely because you are Claude Code, and do not set `--provider codex` merely because the clue looks like a Codex task.

...
SCRIPT_MODULES = ("sync", "commit_trailer", "read_thread", "find_threads", "redact", "jieli_config", "handoff_info")
...
class CodexFindThreadsTests(PluginScriptTestCase):
    ...
    def test_find_threads_includes_provider_only_when_explicit(self):
        ...
        with patch("urllib.request.urlopen", fake_urlopen):
            fetch_threads("payment bug", "https://jieli.example.test", "secret", provider="claude_code")

        query = parse_qs(urlparse(captured["url"]).query)
        self.assertEqual(query["provider"], ["claude_code"])
...
class CodexPluginManifestTests(PluginScriptTestCase):
    ...
    def test_public_docs_describe_split_jieli_skills_without_self_hosted_examples(self):
        ...
        self.assertNotIn("Jieli thread reading skill", docs)
